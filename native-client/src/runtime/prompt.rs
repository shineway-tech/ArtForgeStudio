...
pub(super) fn append_category_generation_instruction(
    ...
    if language == PromptLanguage::Chinese {
        format!(
            "{prompt}\n\nUI 组件图集规则（必须遵守）：只生成一张高密度、可复用的游戏 UI 组件图集，根据用户提示词决定主题、材质和配色。如果用户提示词和已选择的风格都没有明确指定视觉风格，默认采用 2D 手绘游戏 UI 风格，使用清晰轮廓、手绘体积明暗、轻微倒角和统一材质表现；如果用户明确指定像素、扁平、赛博、写实或其他风格，则优先服从用户指定，不要强制使用默认风格。默认生成 32 至 48 个彼此独立且有明显差异的 UI 组件，按画布比例组织为约 5 至 8 列和多行，紧凑覆盖约 85% 的有效画布区域。组件集合必须丰富且有层次，并至少覆盖以下 11 类，每类至少出现一组：角色头像、血条或能量条、按钮、通用图标、虚拟摇杆、物品格、技能图标、货币、设置图标、小地图、宝箱。还可以按用户主题补充状态标记、钥匙、消耗品和面板边框；不要只生成少数大型组件。把已选择的 UI 子类型理解为组件用途，而不是完整界面。所有组件使用正视图、完整清晰边界、统一风格和一致尺度，组件之间保留清晰窄间隔，不重叠、不裁切。不要生成游戏场景、角色全身、环境、设备样机、完整界面、应用截图、说明文字、标签或水印。使用均匀的纯色中性背景；如果模型支持透明通道，则优先透明背景，以便后续逐个抠图拆分。即使用户提示词提到场景、游戏画面或完整界面，也必须优先遵守本规则，只输出独立 UI 组件图集。"
        )
    } else {
        format!(
            "{prompt}\n\nUI component atlas rule (mandatory): Generate exactly one dense, reusable game UI component asset sheet. Use the user's prompt only to determine the theme, materials, and colors. If neither the user's prompt nor the selected style specifies a visual style, default to a 2D hand-painted game UI style with crisp silhouettes, painted volumetric shading, subtle bevels, and coherent material rendering. If the user explicitly requests pixel art, flat design, cyberpunk, realism, or any other style, follow that request instead of forcing the default. Generate 32 to 48 distinct isolated UI components by default, arranged in roughly 5 to 8 columns with multiple rows according to the canvas aspect ratio, compactly filling about 85% of the usable canvas. The set must be varied and hierarchical and must include at least one group from each of these 11 categories: character portraits, health or energy bars, buttons, general icons, a virtual joystick, inventory slots, skill icons, currencies, a settings icon, a minimap, and treasure chests. Add status markers, keys, consumables, and panel frames when relevant to the user's theme; never return only a few oversized components. Treat the selected UI subtype as the purpose of the component set, not as a request for a complete screen. Show every component front-facing with complete crisp boundaries, consistent style and scale, and narrow clear gutters; no overlap and no cropping. Do not render gameplay scenes, full-body characters, environments, device mockups, complete screens, app screenshots, explanatory text, labels, or watermarks. Use a uniform neutral solid background, or a transparent background when alpha output is supported, so each component can be extracted later. This rule overrides any request for a scene, gameplay image, or complete interface."
        )
    }
}
