...
| F13 | 多个精确比例与后端三类比例不一致 | 首版 UI 只展示“方形/横图/竖图” |
...
- dev 默认 API 地址：`http://127.0.0.1:3000`。
...
后端暂时只支持 `square`、`landscape`、`portrait`，因此首版 UI：

- 显示“方形”“横图”“竖图”，不继续承诺精确比例。
- 对应发送 `square`、`landscape`、`portrait`。
- 张数限制为 1–4。
- 画质使用 `1K`、`2K`、`4K`。
- 是否可选由服务端模型目录和会员权益决定。
...
- `npm run api:check` 通过：以 Mock 邮件验证码和支付宝完成真实 HTTP 路由的注册登录、500 初始积分、账户、设备会话、会员、积分、模型目录、任务与通知读取、提示词任务创建/查询/取消、会员下单、积分包下单、支付状态同步、refresh token 轮换、退出和退出后 401；测试账号、订单、任务和连接自动清理。
...
- 图片任务粒度：分别验证清晰度和数量缺失、8K、数量 0/5、非法比例、9 张参考图、非法 UUID、重复参考图、免费会员请求 2K；同时验证 square/landscape/portrait 三种成功值、两次 4 图任务预占 400 积分、第三次额度不足，以及取消后恢复为可用 500/预占 0。
...
- 扩展测试时确认注册赠送积分受 IP 日配额控制；Mock 服务现为每个请求注入独立测试 IP，并在退出时清理配额桶，避免测试顺序造成赠送积分从 500 变为 0，也不影响真实 dev 配额。
...
- 2026-07-14：首版前端只展示方形、横图、竖图，避免假装支持精确比例。
