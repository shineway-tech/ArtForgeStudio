...
注册、配额占用、风控记录、500 积分批次和账本记录必须在同一事务中完成。
...
|---|---:|---:|---:|---|
| free | 0 | 0 | 10000 | 1K |
| basic | 2900 | 3000 | 9500 | 2K |
| advanced | 6900 | 8000 | 9000 | 4K |
| professional | 15900 | 20000 | 8500 | 4K |

免费注册赠送 500 积分不放在免费套餐版本中，由 `registration_grants` 单独控制。
...
7. 符合条件时在同一事务增加两个桶的 `granted_count`，创建 500 积分、30 天有效的 `credit_lots`，并写入 `credit_ledger(grant)`。
