...
更新清单中的下载 URL 使用版本路径，并为每个平台记录文件大小与 SHA-256。客户端在后台下载完成后必须同时校验两者，校验通过后才允许启动安装流程。
...
## Desktop website payment acceptance

使用发布候选后端和支付宝沙箱或正式小额订单执行：

1. 登录后分别创建积分充值和会员订单。
2. 客户端先显示等待支付窗口，再用系统默认浏览器打开支付宝网站；不得创建内嵌支付 WebView。
3. 客户端只允许打开当前 API 同源 HTTPS 中转页，由服务端校验短期会话并顶层跳转到支付宝。
4. 关闭等待支付窗口只隐藏界面，不取消服务端订单；顶部“查看支付状态”可重新打开窗口，客户端继续同步状态。
5. `paid` 且权益仍在重试时显示“权益处理中”，不能降级为失败。
6. 公网异步回调完成后，积分或会员到账、通知更新并清理待恢复订单。
7. 重启客户端后，未完成订单仍可恢复。

验收记录包含 Windows、macOS、系统浏览器、Actions 工作流链接、订单号和服务端请求标识；不得记录 Token、签名 URL、支付 URL 或密钥。
